...
package com.praisehim.koraildelay.SwipeLibrary;
...
import com.praisehim.koraildelay.InterestTrain;
import com.praisehim.koraildelay.R;
...
//				swipeListView.onClickFrontView(position);
//				
//			}
//		});
	...
	private void generateRevealAnimate(final View view, final boolean swap,
			final boolean swapRight, final int position)
	{
		TrainInfo thisTrain = Data.TRAINLIST.get(position);
		
		thisTrain.isOpen = !thisTrain.isOpen;

		if(thisTrain.isOpen)
		{//////////////////
			
		TextView trainRegistTextView = (TextView) backView.findViewById(R.id.trainRegistTextView);
		LinearLayout backViewContainer = (LinearLayout) backView.findViewById(R.id.backViewContainer);

		boolean canInstance = InterestTrain.canInterest(thisTrain);
		boolean isExist = false;
		if(canInstance)
		{	Log.e("스와이핑됨", "이것이 나왔다는 말은 현재 아이템은 생성가능");
			isExist = InterestTrain.isExist(thisTrain);
			
			if(!isExist)
			{
				InterestTrain.addInterest(thisTrain);
				trainRegistTextView.setText("해당 열차가\n등록되었습니다");
				backViewContainer.setVisibility(View.VISIBLE);
				backView.setBackgroundColor(backView.getResources().getColor(android.R.color.holo_blue_light));
			}
			else
			{
				InterestTrain.removeInterest(thisTrain);
				backViewContainer.setVisibility(View.GONE);
				backView.setBackgroundColor(backView.getResources().getColor(android.R.color.holo_orange_light));
				trainRegistTextView.setText("해당 열차의 등록이\n해제되었습니다");
			}
		}
		else
		{
			Log.e("스와이핑됨", "현재 아이템은 생성불가능");
			trainRegistTextView.setText("이미 출발한 열차는\n등록할 수 없습니다");
			backViewContainer.setVisibility(View.GONE);
			backView.setBackgroundColor(backView.getResources().getColor(android.R.color.holo_orange_light));
		}
		...
		if(Data.INTEREST_LIST!=null)
			Log.e("리스트개수", "" + Data.INTEREST_LIST.size());
...
//		}
		...
		animate(frontView).translationX(moveTo).alpha(Math.max(0.3f, Math.min(1f, 1f - 2f * Math.abs(moveTo)
				/ viewWidth))).setDuration(animationTime)
				.setListener(new AnimatorListenerAdapter()
				{
					@Override
					public void onAnimationEnd(Animator animation)
					{
						...
						if (swap)
						{
							...
							if (aux)
							{
								Log.e("animationTime", String.valueOf(animationTime));
								animate(backView).alpha(1)
									.setDuration(400);
	...
	public boolean onTouch(View view, MotionEvent motionEvent)
	{
		...
		case MotionEvent.ACTION_DOWN:
		{
			...
			for (int i = 0; i < childCount; i++)
			{
				...
				boolean allowSwipe = swipeListView.getAdapter().isEnabled(
						childPosition)
						&& swipeListView.getAdapter().getItemViewType(
								childPosition) >= 0;
...
//				frontView.setLongClickable(opened.get(downPosition));
		...
		case MotionEvent.ACTION_MOVE:
		{
					...
					&& velocityY < velocityX)
			{
				...
				Log.d("SwipeListView", "deltaX: " + deltaX
						+ " - swipingRight: " + swipingRight);
				if (opened.get(downPosition))
				{
	...
	public void move(float deltaX)
	{
		...
		if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS)
		{
			...
			setAlpha(
					frontView,
					Math.max(0.3f, Math.min(1f, 1f - 2f * Math.abs(deltaX)
							/ viewWidth)));
